...
**Triage run date:** 2026-07-13; **refreshed 2026-07-17**
**Source of truth:** Jira (xyzreality.atlassian.net) + hc-frontend `claude/vigilant-franklin-cs6txw` + live-incident-playbook.md

> **07-17 refresh, top-line:** One new Jira comment since the last pass — **Darminder
> Atker, 2026-07-13 18:27 BST** (the 07-13 pass ran earlier that day and missed it).
> It is a **root-cause diagnosis** ("legacy *dashboard only* role from old-nav
> invites should map to `viewer_role`/Viewer; authorities preset in BE, users can't
> edit; V1 no longer supported"), NOT a customer-facing ask and NOT a status change.
> Net: (1) the root cause is now **on the ticket** (was previously off-Jira only);
> (2) the diagnosis points the fix toward **correcting the users' role data / cohort
> remap**, not toward changing the FE gate; (3) still **no stated customer ask**, no
> cohort-sweep or code-fix ticket, no trigger correlation, status unchanged. The FE
> gate is **still unfixed** (re-verified in code, now at `project-private-route.tsx:49`).
> Nothing has moved in the 4 days since Darminder's comment — **stalled, assignee
> Darminder, nobody visibly driving it.**

---

## 1. Ticket snapshot (as of 17 Jul 2026)
...
| Type | Live Incident |
| Priority | **Blocker** |
| Status | **With Customer** (category: In Progress) — set 2026-07-10 14:34 by Darminder Atker, straight from *Open*, **no comment**; **still With Customer as of 17 Jul** (Darminder's 13 Jul comment did NOT change status and did NOT explain the customer-wait) |
...
### Comment thread (5 comments; 4 on 8 Jul + **1 new on 13 Jul**; the real investigation lived in a chat thread, not here)
1. 17:23 (8 Jul) Yash → Darminder: "Jira for record."
2. 17:48 (8 Jul) Sergey: asks for user email + error screenshot.
3. 18:01 (8 Jul) Yash: Freshdesk 7374 → Waiting on 3rd line.
4. 18:03 (8 Jul) Yash: email = mbowser@switch.com + two inline error images (blob:… — **not retrievable ⚠️**).
5. **18:27 (13 Jul) Darminder Atker — NEW since last pass** (posted the evening of the 07-13 triage run, after it completed). Verbatim: *"Source of the problem appears to be users have been invited previously with old navigation and set with 'dashboard only' role. Users with this role should have: `userRoleCode: viewer_role`, `userRoleName: Viewer`. The authorities for this role are preset in the backend and users cannot edit. Now that new navigation is being used and V1 is no longer being supported."*

**What comment 5 does and doesn't do:**
- ✅ **Puts the root cause on the ticket** — previously it lived only in the off-Jira chat thread / the playbook. Matches the playbook root cause (legacy "Dashboard Only" role, single authority `DashboardView`).
- ✅ **New trigger hint (not confirmed):** *"Now that new navigation is being used and V1 is no longer being supported"* — the **V1 / old-navigation deprecation** is now the leading candidate for "why now", though still not correlated to a dated deploy/error-rate. Raises trigger confidence modestly (6 → 6-7).
- ⚠️ **Points the fix at the DATA/cohort side, not the gate:** framing is "these users *should* be Viewer" (i.e. correct their role mapping) — i.e. the cohort-remap / retire-legacy-role branch, NOT "honor `DashboardView` in the FE gate". Useful directional signal for the honor-vs-retire decision.
- ❌ **Does NOT explain the "With Customer" status** — no customer-facing ask, no note on what SWITCH owes us. The single biggest record gap from the last pass **remains open**.
- ❌ No cohort-sweep ticket, no code-fix ticket, no owner assigned to the trigger question, no confirmation mbowser can now access.

**No comment after 13 Jul 18:27.** In the 4 days since, nothing has moved — assignee is Darminder, but nobody is visibly driving it. The move to "With Customer" (10 Jul) still carries no explanation.
...
### What it is waiting on the customer for — **STILL UNDOCUMENTED in the ticket** ⚠️ (unchanged 07-17)
The ticket was flipped to "With Customer" by the FE lead with no note, and no comment states the ask. Darminder's **13 Jul comment is a diagnosis, not a customer ask** — it does not close this gap. **Inferred** (from sibling-ticket patterns + playbook): after the role→Viewer flip / Redis cache flush that resolved affected users on 9 Jul, the team is waiting for SWITCH to **confirm mbowser can now open the dashboards** (mirrors PAPI-3613's "Could you please check it with the customer?" → "Waiting on customer", and PAPI-3621's "is the issue still happening?"). **This is inference, not stated fact — confidence 4.** Confirming the exact ask is still the single biggest gap in the record (drives the recommended action). Note the tension: if the fix is really a **role-data remap** (per Darminder's comment) rather than something the customer must verify, "With Customer" may simply be a **mis-parked ticket** — the ball may actually be with XYZ (Darminder/BE), not SWITCH.
...
|---|---|---|---|
| 1 | **Trigger unknown** — correlate access-denial error rate with deploy timeline ("why now") | **STILL OPEN — but a candidate named (07-17)** | Darminder's 13 Jul comment offers the first on-ticket trigger hypothesis: *"now that new navigation is being used and V1 is no longer being supported"* → the **V1 / old-navigation deprecation** is the leading "why now" candidate. Still **not correlated** to a dated deploy or error-rate curve, so unconfirmed. Consistent with the earlier datum that the FE gate itself (`project-private-route.tsx`, `userAuthorityUtils.ts`) was last touched by PLT-2753 (#1959) pre-incident — i.e. the trigger is a nav/role-lifecycle change, not a gate-code deploy. Conf 6→**6-7**. |
...
## 4. hc-frontend code check (branch `claude/vigilant-franklin-cs6txw`; Commissioning OFF — no marker, non-commission branch)

> **07-17 re-verification:** the FE dashboard gate is **still not honoring `DashboardView`** — re-read live in code. The gate has **moved from line 41 to line 49** (the file gained a `useProjectAccessGuard`/`accessRevoked` block + a react-query `projectAuthoritiesQueryConfig` fetch above it), but the accepted-authority set is **unchanged**: `hasAnyAuthorities={[AUTHORITIES.PROJECT_VIEW, AUTHORITIES.PROJECT_EDIT]}` — `DASHBOARD_VIEW` still absent. **Root-cause FE fix is still outstanding.** (Cite `project-private-route.tsx:49` going forward, not :41.)
...
### (b) FE dashboard route gate — **`DashboardView` NOT honored ❌ (root-cause FE fix NOT applied)** — re-verified 07-17
...
- `ProjectPrivateRoute` gates on **`[AUTHORITIES.PROJECT_VIEW, AUTHORITIES.PROJECT_EDIT]`** — `src/main/webapp/app/shared/auth/project-private-route.tsx:49` (**was :41** — line shifted, authorities unchanged). `DASHBOARD_VIEW` is **absent** from the accepted set.
...
| FE dashboard gate still requires ProjectView; DashboardView not honored (re-verified 07-17, now :49) | 9 |
| PLT-2879 scope = customer access incident (token race only attributed by commit) | 8 |
| Root cause now recorded ON the ticket (Darminder 13 Jul comment matches playbook) | 9 |
...
| Follow-up #1 trigger STILL OPEN — V1/old-nav deprecation now leading candidate (uncorrelated) | 6-7 |
| Follow-up #2 cohort sweep STILL OPEN/UNKNOWN (Darminder's comment leans toward this branch) | 4 |
| **What PLT-2879 is waiting on the customer for** (still inferred; undocumented — possibly mis-parked) | **4** |
| **Ticket is stalled — no forward motion in 4 days since diagnosis, nobody driving** | **7** |
