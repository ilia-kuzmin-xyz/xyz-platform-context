...
- **Attachments:** 2 × `.mp4` screen recordings + 2 inline blob images in comment 1 (all unreadable here — see NEEDS HUMAN) · **+ `activity-7c4f2509-orphaned-418.csv`** (id 60732, `text/csv`, ~30 KB, Ilia Kuzmin, 2026-07-14 — the enumerated list of 418 dead links proposed for deletion; content behind Atlassian auth, see NEEDS HUMAN)
...
- ⚠️ **Two inline blob images in comment 1** (`media.staging.atl-paas.net` blobs) — the two screenshots Yash embedded. Not resolvable here.
  - The videos/screenshots are the only way to confirm **which** failure mode the user hit: menu item *greyed out* (all elements orphaned) vs *clickable but no effect* (elements resolve but their model isn't loaded / filtered out). This distinction matters for the fix. (Note: the 2026-07-14 diagnostic run has since *superseded* the need for the videos — it proved the "clickable, resolves in registry, 0 in geometry" mode directly. The videos are now only nice-to-have.)
- ⚠️ **`activity-7c4f2509-orphaned-418.csv`** (attachment id **60732**, `text/csv`, **30,957 bytes ≈ 30 KB**, Ilia Kuzmin, 2026-07-14 17:53:34+01:00) — the enumerated list of the 418 dead `activity_links` proposed for deletion (attached to comment 107357, "here's the list of linkings to be removed"). **Attempted to read it (curl + WebFetch + MCP fetch) — all returned HTTP 403** (no delegated auth to attachment content from this environment). **Flagged NEEDS HUMAN — but PRIORITISE this one:** unlike the `.mp4`s (which need someone to actually watch them), this is a **plain CSV** a human can open in one click. A human should (a) confirm the row count = **418** and that it matches the diagnostic's `inParquetNotInGeometry` figure, (b) confirm the columns identify each link unambiguously for deletion (expected: link/element UUID + `sourceFileElementId` + activityId `FAR01UGD1220`), and (c) sanity-check that every row belongs to this one activity before any bulk delete is run against it.
