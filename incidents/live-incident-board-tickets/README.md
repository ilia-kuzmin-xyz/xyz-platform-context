...
| PLT-2917 | progress-tracking | Open (new) | Milestones wrong on the **old PowerBI dashboard** (surface corrected 07-22 with operator: ticket URL = PBI embed `ProgressReportPage`, NOT the native Portfolio widget first analysed — that analysis kept as the verified "FE renders reporting-DB state" half). "Key Milestones" = activities mapped Discipline=`Milestone`+Package=`Key milestone` in the editor; editor side verified correct (DH4 = 100%). Leading unifying hypothesis: **PBI pinned to a stale schedule version/baseline** (schedule name `…updated__I_(3)`) — explains all 3 projects + why Pietro's fix didn't stick | (revised) client-clarification reply via Yash drafted: which dashboard (old/new/both), re-attach broken screenshots, one concrete example per project w/ surface multiple-choice → then move **With Technical Support**; separate internal track: Pietro (what did his fix touch) + Hussein (which dataset/schedule version the PBI milestone visual binds) | 6/10 |
