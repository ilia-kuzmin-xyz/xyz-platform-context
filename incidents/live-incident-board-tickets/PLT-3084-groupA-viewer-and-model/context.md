...
## 2026-08-26 — left scope (→ Ready For QA); folder retagged groupA → resolved

Live JQL fetch shows PLT-3084 has moved **In Analysis → Ready For QA**, with `updated` timestamp
newer than the 08-25 snapshot recorded above (comment content not re-diffed — the routine's
exclusion list treats Ready For QA as out of scope, so no further Jira delta was pulled this run).
Consistent with the 08-24 RESOLVED finding: the actual fix was shipping a build containing PR #2081
rather than any new code, and that appears to now be in the release pipeline behind QA. Folder
